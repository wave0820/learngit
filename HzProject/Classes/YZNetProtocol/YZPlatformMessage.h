...
typedef struct MSG_GP_S_UserSwitch
{
    char								szPwd[50];						//密码暂不用 预留
    int									dwUserID;						//预留
}MSG_GP_S_UserSwitch;
typedef struct MSG_GP_S_UserSwitch_Rsp
{
    bool								bClose;							//关闭标志位
}MSG_GP_S_UserSwitch_Rsp;
#define ASS_GP_USER_SWITCH                                  10		 //逃避审核切换

#define ASS_GP_BROADCAST									3		//广播公告

#define	MDM_GP_MESSAGE					                    103

typedef struct MSG_GP_S_BroadCast
{
    BYTE								bType;								//1 公告  2 新闻   暂填1
}MSG_GP_S_BroadCast;
typedef struct MSG_GP_S_BroadCast_Rsp
{
    char			szNotice[256];
}MSG_GP_S_BroadCast_Rsp;

typedef struct MSG_GP_R_UPDATE_URL
{
    char								szUpdateUrlAndroid[256];
    char								szUpdateUrlIphone[256];
}GNUC_PACKED MSG_GP_R_UPDATE_URL;
