...
#define ASS_GP_BANK_NET_PRE_DEPOSIT             32      //网络预充值 add by wy 20160525
#define HC_GP_BANK_NET_PRE_DEPOSIT_SUC      0       //成功
#define ASS_GP_BANK_NET_DEPOSIT                 33      //网络充值 add by wy 20160525
#define HC_GP_BANK_NET_DEPOSIT_SUC          0       //成功
typedef struct TMSG_GP_BankNetDeposit_Pre
{
    int     _user_id;       //用户ID 必填
    LLONG    _money;           //充值金额  不是虚拟币 必填
    short   _pay_type;      //支付通道类型 1 易宝充值_网银充值  2 聚宝云_网银充值 必填
    char    _type_info[50]; //通道表述 填上面的 中文 与 _pay_type 一一对应 必填
    short   _purchase_type; //充值类型 0 元宝 1 金币 必填
    char    _order_id[50];  //定单号 必填
    char    _phone[15];     //电话 清零即可 暂不使用
    
}GNUC_PACKED TMSG_GP_BankNetDeposit_Pre;

//网络充值   add by wy 2016-5-25
typedef struct TMSG_GP_NetDepositRsp   //充值后的财富变化 成功后从这个结构体里获取变化后的信息
{
    LLONG _money_bank;   //银行钱  虚拟货币
    LLONG _money_change; //充值钱  虚拟货币
    LLONG _money_wallet; //身上钱  虚拟货币
}GNUC_PACKED TMSG_GP_NetDepositRsp;

typedef struct TMSG_GP_BankNetDeposit
{
    int     _user_id;       //用户ID
    char    _order_id[50];  //定单号 必填
    int     _rsp_code;		//支付接口的返回值 必填
    LLONG  _money;          //充值金额  不是虚拟币 清零即可 暂不使用
    short   _pay_type;      //支付通道类型 1 易宝充值_网银充值  2 聚宝云_网银充值  清零即可 暂不使用
    char    _type_info[50]; //通道表述 填上面的 中文 与 _pay_type 一一对应 清零即可 暂不使用
    short   _purchase_type; //充值类型 0 元宝 1 金币 清零即可 暂不使用
    int     _room_id;       //房间id  清零即可 暂不使用
    TMSG_GP_NetDepositRsp _rsp;
}GNUC_PACKED TMSG_GP_BankNetDeposit ;
