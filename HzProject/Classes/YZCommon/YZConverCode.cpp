...
#define GAME_AUDIO_ROOT "erMaJiang/music/"
...
std::string YZConverCode::GB2312ToUtf8(const char *utf8)
{
    ...
    std::string YZConverCode::FormatSoundFileByLanguage(const char *_fileName){
        std::string filename(GAME_AUDIO_ROOT);
        YZAudioEngine::getInstance()->getSoundLanguage() == 2 ? filename.append("wuhu/") : filename.append("china/");
        filename.append(_fileName);
        CCLOG("Format sound file name : %s" , filename.c_str());
        return filename;
    }
