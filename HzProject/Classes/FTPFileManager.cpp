...
int FTPFileManager::uploadFile2ServerByFtp(std::string fileName , std::function<void(std::string fileName)> _uploadCallBack){
    CURL *curl;
    CURLcode res;
    FILE *hd_src;
    ...
    if(curl) {
        ...
        if(res != CURLE_OK){
            ...
            //XLMahjong::GameTableUI::getInstance()->uploadFileEnd(fileName);
        }else{
            //XLMahjong::GameTableUI::getInstance()->uploadFileEnd(fileName);
            if (_uploadCallBack != nullptr) {
                _uploadCallBack(fileName);
...
void FTPFileManager::downloadFile2LocalByFtp(std::string _fileName , std::function<void(std::string fileName)> _downloadCallBack){
    ...
    if(curl) {
        ...
        if(CURLE_OK != res) {
            ...
            if (_downloadCallBack != nullptr) {
                _downloadCallBack(_fileName);
            }
            
            //XLMahjong::GameTableUI::getInstance()->downloadFileEnd(_fileName);
        }
    }
