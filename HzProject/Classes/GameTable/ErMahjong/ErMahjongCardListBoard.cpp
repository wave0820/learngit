...
namespace ErMahjong
{
        ...
        switch (_direction) {
            case ErMahjong::MJPostion::Bottom :
            {
                ...
                for (int i = 0; i < MAX_COUNT; i++) {
                    ...
                    _cardPoint[i] = Vec2(MJ_BOTTOM_W  * (i+1) - MJ_BOTTOM_W/1.5f, this->getPositionY());
                    if (i == MAX_COUNT - 1) {
                        _cardPoint[i] +=Vec2(MJ_BOTTOM_W/1.5f , 0);
                    }
                }
            }
                break;
            ...
            {
                for (int i = 0; i < MAX_COUNT; i++) {
                    _cardPoint[MAX_COUNT - i - 1] = Vec2(this->getPositionX(),size.height * 0.28f +  MJ_RIGHT_H /2 * (i + 1));
                    
                    if (i == 0) {
                        _cardPoint[MAX_COUNT - 1] -= Vec2(0,MJ_RIGHT_H/2);
                    }
                }
                ...
                for (int i = 0; i < MAX_COUNT; i++) {
                    ...
                    if (i == MAX_COUNT - 1) {
                        _cardPoint[i] = Vec2(this->getPositionX(),size.height * 0.75f - MJ_LEFT_H/2 * (i + 1) - MJ_LEFT_H/2);
                    }else{
                        _cardPoint[i] = Vec2(this->getPositionX(),size.height * 0.75f - MJ_LEFT_H/2 * (i + 1));
                    }
                }
            ...
            {
                for (int i = 0; i < MAX_COUNT; i++) {
                    _cardPoint[i] = Vec2(size.width * 0.82f - MJ_TOP_W * (i + 1), size.height/2 - 50);
                    if (i == MAX_COUNT - 1) {
                        _cardPoint[i] -= Vec2(MJ_TOP_W / 2 , 0);
                    }
                }
            }
                break;
            ...
            {
                for (int i = 0; i < MAX_COUNT; i++) {
                    _cardPoint[i] = Vec2(this->getPositionX(),size.height * 0.26f +  MJ_RIGHT_H /2 * (i + 1));
                    if (i == MAX_COUNT - 1) {
                        _cardPoint[i] += Vec2(0,  MJ_RIGHT_H/2);
                    }
                }
                this->setContentSize(Size(MJ_RIGHT_W , size.height));
            ...
            if (_outCardIndex == cardNeed2Index  && cardNeed2Index != MAX_COUNT - 2) {
                ...
                if (cardNeed2Index == MAX_COUNT - 2) {
                    ...
                    for (int i =  cardNeed2Index ; i >= _outCardIndex; i--) {
                        ...
                        tmpCard->runAction(Sequence::create(MoveTo::create(length/MJ_W_MOVE_SPEED, toPoint),CallFunc
                                                            ::create([=]{
                            _isOuting = false;
                        }),NULL));
                    }
                    ...
                    auto fun = CallFunc::create([=]{
                        ...
                        if (_needMoveCount <= 0) {
                            ...
                            lastCard->runAction(Sequence::create(MoveTo::create(0.1f,lastCard->getPosition() + Vec2(0, MJ_BOTTOM_H)),MoveTo::create(0.3f, toPoint + Vec2(0, MJ_BOTTOM_H)),MoveTo::create(0.1f, toPoint),CallFunc::create([=]{
    ...
    void CardListBoard::cardMove2Index(MahjongCard* card ,int index){
        ...
        card->runAction(Sequence::create(MoveTo::create(0.1f, card->getPosition() + offset),MoveTo::create(0.3f, toPoint + offset),MoveTo::create(0.1f, toPoint),CallFunc::create([=]{
    ...
    bool CardListBoard::onTouchBegan(Touch* touch, Event *event)
    {
        ...
        if(rect.containsPoint(pos))
        {
            ...
            if(nowTouchIndex >= 0 && nowTouchIndex < MAX_COUNT){
                ...
                _selectCardIndex = nowTouchIndex;
                return true;
            }
    ...
    {
        Vec2 pos = this->convertToNodeSpace(touch->getLocation());
        _cardsPtr[_selectCardIndex]->setPosition(pos);
    }
    ...
    void CardListBoard::onTouchEnded(Touch* touch, Event *event)
    {
        ...
        if (pos.y > OUT_CARD_Y && _canOut) {
            ...
            _outCardIndex = _selectCardIndex;
            ...
            _cardsPtr[_selectCardIndex]->setPosition(_cardPoint[_selectCardIndex] + Vec2(0, ERUP_OFFSET));
            
            this->downCard(_cardsPtr[_selectCardIndex]);
            ...
            switch (actionType) {
                ...
                case ActionType::WIK_GANG:  //杠
                {
                    ...
                    if (bDarkGuang) {
                        ...
                        MahjongCard * card4 = _cardsPtr[cardIndexs.at(3)];
    ...
    void CardListBoard::addGuangCard(MahjongCard * card){
        ...
        Point p = Vec2(card->getContentSize().width / 2 , card->getContentSize().height/2);
        gCard->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
        gCard->setScale(1.0f);
        switch (_direction) {
            case Bottom:
                p += Vec2(0, 15);
        ...
        for (int i = 0; i < _cardsPtr.size(); i++) {
            ...
            if (i< _cardStartIndex) {
                ...
                if (_direction == MJPostion::Right) {
                    ...
                    p -= Point(0 , MJ_RIGHT_H/2);
                }else if(_direction == MJPostion::Left){
                    _cardsPtr[i] -> setLocalZOrder(i);
                    p += Point(0 , MJ_RIGHT_H/2);
                }else if(_direction == MJPostion::Bottom){
                    p -= Point(MJ_BOTTOM_W/4 , 0);
                }else{
                    p += Point(MJ_BOTTOM_W/4 , 0);
                }
    ...
    void CardListBoard::showAllWinCard(bool isWin , BYTE winCardValue , const std::vector<BYTE> values){
        ...
        for (int i = _cardStartIndex ; i < _cardsPtr.size(); i++) {
            MahjongCard * card = _cardsPtr[i];
            card->setCardValue(values.at(i - _cardStartIndex));
            card->showCardWinTexture();
        }
        
        /*
        int index = 0;
        bool clearWinCard = true;
        for (int i = _cardStartIndex; i < _cardsPtr.size(); i++) {
            ...
            if (isWin && clearWinCard && values.at(index) == winCardValue) {
                index++;
                clearWinCard = false;
            }
    ...
    void CardListBoard::setCanOutCard(bool iCan)
    {
        ...
        if (_isMySelf) {
            ...
            if (_canOut) {
                _isOuting = false;
                //CCLOG("到我出牌... 设置正在出牌为FALSE..");
            }
            
            if (outCardLine == NULL) {
                Size size = Director::getInstance()->getWinSize();
                outCardLine = Sprite::create("erMaJiang/tuopai.png");
                outCardLine->setPosition(Vec2(size.width/2, OUT_CARD_Y));
