...
namespace ErMahjong
{
		...
		case SUB_S_SEZI:
            ...
            case ASS_GR_DISMISS_BACKROOM :
            {
                ...
                if (_currentGameTime > 0) {
                    //如果是最后一局游戏结束,则表示所有游戏全部结束。 先显示结束界面，然后再查询总游戏结果
                    if (_isLastGameEnd) {
                        
                    }else{
                        //否则表示在游戏过程中强制解散房间。 则直接查询游戏总记录
            ...
            case SUB_S_SHARE_GAME_RECORD:
                dealSeachGameRecordResp(object ,objectSize);
                break;
    ...
    void GameTableLogic::dealSeachGameRecordResp(void * object , INT objectSize){
        JzLog::shareLog()->log(kLogLevelDebug, "==============dealSeachGameRecordResp===============");
        CMD_S_GameRecord* pData = (CMD_S_GameRecord*)object;
        //CCLOG("dealSeachGameRecordResp objectSize : %lu" , sizeof(CMD_S_GameRecord));
        BYTE mySeatNo = logicToViewSeatNo(pData->wMeChair);
        GameRecordData data;
        data.mySeatNo = mySeatNo;
        data.m_dwRoomOwner = pData->m_dwRoomOwner;
        LLONG maxScore = 0;
        BYTE maxScoreSeat = -1;
        for (int i = 0; i< PLAYER_COUNT; i++) {
            BYTE seatNo = logicToViewSeatNo(i);
            data.dwUserID[seatNo] = pData->dwUserID[i];
            data.huTimes[seatNo] = pData->HuNum[i];
            data.gangTimes[seatNo] = pData->GangLight[i];
            data.drakGangTimes[seatNo] = pData->GangDark[i];
            data.zmTimes[seatNo] = pData->ZaNum[i];
            data.score[seatNo] = (int)pData->i64TotalScore[i];
            pData->szNickName[i][60] = '\0';
            
            for (int j = 0; j < 60; j++) {
                pData->szNickName[i][j] = pData->szNickName[i][j];
            }
            
            if (pData->i64TotalScore[i] > maxScore) {
                maxScore = pData->i64TotalScore[i];
                maxScoreSeat = seatNo;
            }
        }
        data.destOwer = getDeskOwner();
        data.winerSeatNo = maxScoreSeat;
        data.roomPwd = RoomLogic()->getInRoomNum();
        data.m_cbZaCodeNum = RoomLogic()->getRoomZMCount();
        
        _uiCallback->showGameAllRecord(data, true);
    }
    ...
    void GameTableLogic::dealListenerCardResp(void * object ,INT objectSize){
        ...
        BYTE seatNo = logicToViewSeatNo(pData->wListenUser);
	...
	void GameTableLogic::dealUserAgreeResp(MSG_GR_R_UserAgree* agree)
    {
        ...
        if (agree->bAgreeGame == 1) {
            UserInfoStruct* pUser = _deskUserList->getUserByDeskStation(agree->bDeskStation);
            
            _uiCallback->playUserReadyEffect(pUser->bBoy);
        }
        ...
        for(int i = 0; i < PLAYER_COUNT; i++)
        {
            ...
            if(_existPlayer[i] && pUser != nullptr)
            {
                ...
                if (pUser->bUserState == USER_ARGEE)
                {
                    ...
                    _userReady[i] = true;
                    _uiCallback->showUserReady(seatNo, pUser->bBoy,true);
                }
            }
        }
	...
	void GameTableLogic::dealUserSitResp(MSG_GR_R_UserSit * userSit, UserInfoStruct* user)
	{
        ...
        if (_gameStatus == GS_PLAYING) {
            JzDebug("游戏正在进行中... 玩家重连游戏..");
            BYTE seatNo = logicToViewSeatNo(userSit->bDeskStation);
            _uiCallback->showUserOffLine(seatNo, false);
        }
    ...
    void GameTableLogic::onUserCutMessage(INT dwUserID, BYTE bDeskNO, BYTE	bDeskStation){
        JzDebug("捕捉到用户断线消息...");
        BYTE seatNo = logicToViewSeatNo(bDeskStation);
        
        if (bDeskNO == _deskNo && seatNo != 0) {
            _uiCallback->showUserOffLine(seatNo, true);
        }
    }
    ...
    void GameTableLogic::dealUserChatMessage(void* object, INT objectSize){
        ...
        {
            _uiCallback->showUserChatTalkMsg(0,t_seat, pData->szMessage);
        }
        ...
        }else if(pData->nDefaultIndex == 3){
            _uiCallback->showUserChatTalkMsg(3,t_seat, pData->szMessage);
        }
    }
    ...
    void GameTableLogic::dealGameStationResp(void* object, INT objectSize)
	{
		...
		switch(_gameStatus)
	    {
            case GS_WAIT_SETGAME:
            {
                ...
                _currentGameTime = pData->m_iPlayGameRounds;
                _isLastGame = pData->m_iPlayGameRounds == pData->m_iTotalGameRounds;
                LLONG i64TotalScore[GAME_PLAYER];
                memset(i64TotalScore, 0, sizeof(i64TotalScore));
                for (int i = 0; i < PLAYER_COUNT; i++) {
                    BYTE localSeatNo = logicToViewSeatNo(i);
                    i64TotalScore[localSeatNo] = pData->i64TotalScore[i];
                }
                
                string pwd = "";
                for(int i = 0; i < 6; i++)
                {
                    pwd += pData->m_szRoomPwd[i];
                }
                
                RoomLogic()->setDeskOwnerInfo(pData->m_dwRoomOwner);
                _uiCallback->updateDeskOwnerId(pData->m_dwRoomOwner);
                _uiCallback->changePlayerScore(i64TotalScore);
                _uiCallback->isLastGame(pData->m_iPlayGameRounds , pData->m_iTotalGameRounds);
                _uiCallback->playerBackGameFreeState(pwd, pData->m_cbZaCodeNum);
            }
                break;
...
#pragma mark - 断线重连
    void GameTableLogic::dealGoOnGameResp(void* object, INT objectSize)
    {
        ...
        data.cbLeftCardCount = pData->cbLeftCardCount;
        data.cbDaCardData = pData->cbDaCardData;
        data.m_cbZaCodeNum = pData->m_cbZaCodeNum;
        ...
        cbHandCardsMaps.clear();
        cbWeaveItemMaps.clear();
        cbHandCardsMaps.clear();
        JzLog::shareLog()->log(kLogLevelDebug, "-------恢复牌值开始----------");
        
        //memset(data.cbDusCardMaps, 0, sizeof(data.cbDusCardMaps));
        //memset(data.cbHandCardsMaps, 0, sizeof(data.cbHandCardsMaps));
        //memset(data.cbWeaveItemMaps, 0, sizeof(data.cbWeaveItemMaps));
        memset(data.i64TotalScore, 0, sizeof(data.i64TotalScore));
        ...
        for (int i = 0; i < PLAYER_COUNT; i++) {
            ...
            cbDusCardMaps[localSeatNo] = outCards;
            ...
            cbWeaveItemMaps[localSeatNo] = weaveItems;
            ...
            if (localSeatNo != 0) {
                ...
                cbHandCardsMaps[localSeatNo] = handCards;
            }
            
            data.i64TotalScore[localSeatNo] = (int)pData->i64TotalScore[i];
        ...
        _caiShenCard = pData->cbDaCardData;
        ...
        for (int i = 0; i< pData->cbCardCount ; i++) {
            ...
            if (cbColor < 0 || cbColor > 3 || cbValue < 1 || cbValue > 9) {
                jzError("恢复手牌牌值出错...  牌值为 : %d" , pData->cbCardData[i]);
            }else{
                if (pData->cbCardData[i] == pData->cbDaCardData) {
        ...
        //log("玩家手牌列表数量为 : %d" , cbCardDatas.size());
        //log("Last Send Card Data.. %d " ,  pData->cbSendCardData);
        bool findSend = false;
        if (wCurrentUser == 0) {
        	//log("轮到自己chu出牌、。。。");
            ...
            if (!findSend) {
            	data.cbSendCardData = cbCardDatas.back();
                cbCardDatas.pop_back();
			}
        ...
        //log("最后玩家手牌列表数量为 : %d" , cbCardDatas.size());

        //cbHandCardsMaps.insert(make_pair(0, cbCardDatas));
        cbHandCardsMaps[0] = cbCardDatas;
        ...
        _uiCallback->updateDeskOwnerId(pData->m_dwRoomOwner);
        //玩家回到游戏
        _uiCallback->playerBackGame(data ,cbDusCardMaps , cbHandCardsMaps ,cbWeaveItemMaps );
        ...
        _uiCallback->setWaitTime(logicToViewSeatNo(pData->wCurrentUser), ErMahjong_Game_Time::OUTCARD_TIME, true, 1);
    }
	...
	void GameTableLogic::dealGameBeginResp(void* object, INT objectSize)
	{
       ...
       //_uiCallback->showAllUserCard(wBankerUser , cards  , pData->cbCardData[MAX_COUNT - 1]);
        ...
        _uiCallback->showGameBanker(wBankerUser, pData->cbCardData[MAX_COUNT - 1], pData->cbUserAction, pData->cbGangCardData[0] , pData->cbRed == _caiShenCard);
        ...
        if (pData->cbActionMask != ActionType::WIK_NULL) {
            if (seatNo == 0) {
                if ((pData->cbActionMask & ActionType::WIK_GANG) != 0) {
                    ...
                    if (pData->cbGangCardCount > 0) {
                        BYTE cardValue = pData->cbGangCardData[0];
                ...
                if ((pData->cbActionMask & ActionType::WIK_CHI_HU) != 0) {
                    ...
                    if (ATUO_WIN_GAME) {
                        if (pData->cbRed == _caiShenCard) {
                            this->sendActRequest(ActionType::WIK_CHI_HU,_caiShenCard);
                        }else{
                            this->sendActRequest(ActionType::WIK_CHI_HU,pData->cbCardData);
                        }
                    }else{
                        if (pData->cbRed == _caiShenCard) {
                            _uiCallback->showActionBtn(seatNo ,pData->cbActionMask , _caiShenCard);
                        }else{
                            _uiCallback->showActionBtn(seatNo ,pData->cbActionMask , pData->cbCardData);
                        }
                    }
                }
             }
...
#pragma 托管
    ...
    void GameTableLogic::dealThisGameEndResp(void *object, INT objectSize){
        ...
        data.bZiMo = false;
        data.bLiuJu = pData->cbLiuJu;
        data.cbZhongZaCardData.clear();
        ...
        for (int i = 0; i < GAME_PLAYER; i++) {
            ...
            if (pData->dwChiHuKind[i] != CHK_NULL) {
                ...
                if ((pData->dwChiHuRight[i] & CHR_ZI_MO)) {
                    data.bZiMo = true;
                }
            }
            
            data.iChangeMoney[localSeatNo] = pData->iChangeMoney[i];
            data.cbHuCard[localSeatNo] = pData->cbHuCard[i];
            ...
            for (int k = 0; k < pData->cbCardCount[i]; k++) {
                ...
                //过滤掉胡牌
                if (passWinCard && pData->cbHuCard[i] != 0 && cardInfo == pData->cbHuCard[i]) {
                    passWinCard = false;
                }else{
            ...
            //将胡牌添加到手牌的末尾
            if (pData->cbHuCard[i] != 0) {
                cardInfos.push_back(pData->cbHuCard[i]);
            }
        ...
        for (int i=0; i < pData->cbZhongCodeNum; i++) {
            data.cbZhongZaCardData.push_back(pData->cbZhongCode[i]);
        }
...
#pragma mark 服务器发送的指令 ---- 动作
    ...
    void GameTableLogic::dealGameActNotifyResp(void* object, INT objectSize)
	{
        ...
        if (pData->cbActionMask != ActionType::WIK_NULL) {
            ...
            if ((pData->cbActionMask & ActionType::WIK_CHI_HU) != 0) {
                ...
                bool isZimo = false;
                if ((pData->dwChiHuRight & CHR_ZI_MO)) {
                    isZimo = true;
                }
                //如果不是自摸，则表示是抢杠胡
                if (isZimo) {
    ...
    void GameTableLogic::dealGameActNotifyResultResp(void *object, INT objectSize){
       ...
       // CHECK_SOCKET_DATA(CMD_S_OperateResult, objectSize, "CMD_S_OperateResult size of error!");
        ...
        if (pData->cbOperateCode != ActionType::WIK_NULL) {
    ...
    int GameTableLogic::getDeskOwner(){
        return RoomLogic()->getDeskOwner();
    }
    ...
    void GameTableLogic::sendGetAllGameRecord(){
        sendData(MDM_GM_GAME_NOTIFY, SUB_C_SHARE_GAME_RECORD);
    }
