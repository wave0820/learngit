...
//////////////////////////////////////////////////////////////////////////
        ...
        switch (_direction) {
            case ErMahjong::MJPostion::Bottom :
            {
                ...
                for (int i = 0; i < MAX_COUNT * 2; i++) {
                    ...
                    _cardPoint[i] = Vec2(size.width * 0.27f + MJ_BOTTOM_W * MJ_BOTTOM_SCALE * (index+1), this->getPositionY() + MJ_BOTTOM_H/2 + MJ_BOTTOM_H/1.5f * hang - (hang * 10));
                    
                    _cardZorderPtr[i] = 5 * (3 - hang) - i;
                    ...
                    this->addChild(sp,_cardZorderPtr[i]);
                    */
                }
            }
                break;
            ...
            {
                int oneCount = 11;
                //int maxCount = oneCount * 2;
                for (int i = 0; i < oneCount * 2; i++) {
                    int hang = i >= oneCount ? 1 : 0;
                    int index = i >= oneCount ? i - oneCount : i;
                    
                    _cardPoint[i] = Vec2(this->getPositionX() + MJ_RIGHT_W * hang - (hang * 5),size.height * 0.25f + MJ_LEFT_H * MJ_LEFT_SCALE * (oneCount - index) + (index * 5));
                    
                    _cardZorderPtr[i] = 3 + i;
            ...
            case ErMahjong::MJPostion::Top :
            {
                for (int i = 0; i < MAX_COUNT *2; i++) {
                    ...
                    _cardPoint[i] = Vec2(size.width * 0.77f - MJ_TOP_W * MJ_TOP_SCALE * (index + 1), this->getPositionY() - MJ_TOP_H * hang + 10 + (hang * 5));
                    
                    _cardZorderPtr[i] = 3*(hang+1) + i;
            ...
            case ErMahjong::MJPostion::Right :
            {
                ...
                for (int i = 0; i < oneCount * 2; i++) {
                    ...
                    int index = i >= oneCount ? i - oneCount : i;
                     ...
                     _cardPoint[i] = Vec2(this->getPositionX() + MJ_RIGHT_W - MJ_RIGHT_W * hang + (hang * 5),size.height * 0.3f + MJ_LEFT_H * MJ_LEFT_SCALE * (index + 1) - (5 * index));
                    
                    _cardZorderPtr[i] = oneCount * 2 - i;
    ...
    void CardOutList::addCard2OutList(MJOutCard card){
        ...
        outIndexSp->runAction(Sequence::createWithTwoActions(DelayTime::create(.6f) , CallFunc::create([=]{
        ...
        switch (_direction) {
            case MJPostion::Bottom:
                outCardPoint = Vec2(size.width/2, _cardPoint[0].y + 30);
                break;
            case MJPostion::Right:
                outCardPoint = Vec2(_cardPoint[0].x - outCard->getContentSize().height/2, size.height/2);
                break;
            case MJPostion::Top:
                outCardPoint = Vec2(_cardPoint[7].x, _cardPoint[0].y - outCard->getContentSize().height/2 - 10);
                break;
            case MJPostion::Left:
                outCardPoint = Vec2(_cardPoint[0].x + outCard->getContentSize().height/2, size.height/2);
                break;
        }
        outCard->runAction(Sequence::create(MoveTo::create(0.1f, outCardPoint),DelayTime::create(0.1f),CallFunc::create([=]{
            string fileName = this->getCardTextureFileName(card.cardValue, _direction);
            ...
            if (zhishiSp!= NULL) {
                zhishiSp->setVisible(true);
                          ...
                          _cardPoint[_cardsPtr.size()]),CallFunc::create([=]{
        
            if (_direction == Right) {
                outCard->setLocalZOrder(70 - _cardsPtr.size());
            }
    ...
    void CardOutList::addAllCard2OutListOneTimes(std::vector<BYTE> cards){
        ...
        for (int i = 0; i < cards.size(); i++) {
            ...
            this->addChild(card,70);
            
            if (_direction == Right) {
                card->setLocalZOrder(70 - i);
            }
