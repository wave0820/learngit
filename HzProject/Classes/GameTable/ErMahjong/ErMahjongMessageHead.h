...
#pragma pack(1)
    ...
    struct CMD_S_StatusFree : GameStationBase
    {
        ...
        LLONG							i64TotalScore[GAME_PLAYER];        //总积分
        char							m_szRoomPwd[50];					//房间密码
        int								m_cbZaCodeNum;
        INT                             m_dwRoomOwner;                          //桌主
    };
    ...
    struct CMD_S_GameStart
    {
        ...
        int								m_iTotalGameRounds;			//◊‹æ÷
        int								m_cbZaCodeNum;
        BYTE							cbGangCardCount;						//扑克数目
        BYTE							cbGangCardData[4];						//扑克数据
        BYTE							cbRed;
    };
    ...
    struct CMD_S_GameEnd
    {
        ...
        BYTE							cbZhongCode[MAX_ZMCOUNT];
        BYTE						    cbZhongCodeNum;
        BYTE							cbHuCard[GAME_PLAYER];        
    };
    
    struct CMD_S_GameRecord
    {
        int								dwUserID[GAME_PLAYER];			   //用户ID
        char							szNickName[GAME_PLAYER][61];		   //用户昵称
        LLONG							i64TotalScore[GAME_PLAYER];        //总积分
        int								HuNum[GAME_PLAYER];			   //胡次数
        int								GangDark[GAME_PLAYER];			   //暗杠条数
        int								GangLight[GAME_PLAYER];			   //明杠条数
        int								ZaNum[GAME_PLAYER];					//中码数
        WORD                            wMeChair;
        INT                             m_dwRoomOwner;                          //桌主
    };
    ...
    struct BackGameData{
        BYTE							cbActionCard;								//动作牌
        BYTE							cbActionMask;								//动作掩码
        BYTE							wCurrentUser;								//当前出牌用户
        BYTE							wBankerUser;								//庄家
        bool							bTrustee[GAME_PLAYER];						//托管状态
        BYTE							wOutCardUser;								//当前出牌用户
        BYTE							cbOutCardData;								//当前打出的牌
        ...
        std::string                     roomPwd;
        BYTE                            cbDaCardData;
        INT                             i64TotalScore[GAME_PLAYER];        //总积分
        int								m_cbZaCodeNum;
    };
    ...
    struct GameRecordData{
        BYTE mySeatNo;
        int dwUserID[GAME_PLAYER];			   //用户ID
        int huTimes[PLAYER_COUNT];
        int gangTimes[PLAYER_COUNT];
        int drakGangTimes[PLAYER_COUNT];
        int zmTimes[PLAYER_COUNT];
        int score[PLAYER_COUNT];
        char szNickName[GAME_PLAYER][61];
        BYTE destOwer;
        BYTE winerSeatNo;
        INT m_dwRoomOwner;
        std::string roomPwd;
