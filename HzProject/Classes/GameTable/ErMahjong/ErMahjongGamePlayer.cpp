...
namespace ErMahjong
{
	...
	const char* Player_Normal_HEAD  = "erMaJiang/DisplayPicture/head_defult.png";
	const char* Player_Auto      = "erMaJiang/DisplayPicture/btn_autoOutCard2.png";
    const char* Player_OFFLINE      = "erMaJiang/DisplayPicture/offLine.png";
    ...
    void PlayerUI::setPlayerOffLine(bool offLine){
        
        if (offLine) {
            _ivHead->loadTexture(Player_OFFLINE);
            _ivHead->setScale(55.0f / _ivHead->getContentSize().width, 55.0f / _ivHead->getContentSize().height);
            _ivHead->setVisible(true);
            _ivHead->setLocalZOrder(2);
        }else{
    ...
    void PlayerUI::setDeskOwnerID(LLONG userId){
        _imgOwner-> setVisible(false);
        if (userId == _userId) {
            _imgOwner-> setVisible(true);
        }else{
            _imgOwner-> setVisible(false);
        }
    }
	...
	void PlayerUI::setHead(UserInfoStruct* userInfo)
	{
        ...
        _ivHead->setVisible(false);
        _textMoney->setVisible(true);
        
        std::string headUrl = userInfo->szHeadUrl;
        auto httpSp = HttpSprite::create(headUrl, Player_Normal_HEAD);
	...
	void PlayerUI::loadComponet(ui::Widget* widget)
	{
        ...
        _ivHead->setVisible(true);
        _ivHead->loadTexture(Player_Normal_HEAD);
        _ivHead->setScale(60.0f/_ivHead->getContentSize().width, 60.0f/_ivHead->getContentSize().height);
        ...
        _imgOwner    = dynamic_cast<ImageView*>(ui::Helper::seekWidgetByName(widget, "Image_owner"));
        _imgOwner->setScale(.7f);
        _imgOwner->setVisible(false);
		//_textName->setString("");
		_textMoney->setString("0");
        _imgBanker->setVisible(false);
        _textMoney->setVisible(false);
