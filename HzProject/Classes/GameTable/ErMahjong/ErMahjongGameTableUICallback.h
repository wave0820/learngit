...
namespace ErMahjong
{
	...
	public:
		...
		virtual void showUserReady(BYTE seatNo,bool boy,bool visible) = 0;
        virtual void playUserReadyEffect(bool isBoy) = 0;
        ...
        virtual void showGameBanker(BYTE seatNo , BYTE bankerCard , BYTE bankerAciton , BYTE actionCard , bool _bankerFourHZ) = 0;
        ...
        virtual void changePlayerScore(LLONG  i64TotalScore[GAME_PLAYER]) = 0;
        
        virtual void playerBackGame(BackGameData data ,std::map<BYTE , std::vector<BYTE>> cbDusCardMaps ,std::map<BYTE , std::vector<BYTE>> cbHandCardsMaps,std::map<BYTE , std::vector<CMD_WeaveItem>> cbWeaveItemMaps)= 0;
        
        virtual void playerBackGameFreeState(std::string pwd , int zmCount) = 0;
        
        virtual void updateDeskOwnerId(int userId) = 0;
        //virtual void playerBackGame(BackGameData data ,std::map<BYTE , std::vector<BYTE>> cbDusCardMaps,std::map<BYTE , std::vector<BYTE>> cbHandCardsMaps,std::map<BYTE , std::vector<CMD_WeaveItem>> cbWeaveItemMaps)= 0;
        ...
        virtual void showUserChatTalkMsg(int type , BYTE seatNo, std::string msg) = 0;
        
        virtual void showGameAllRecord(GameRecordData data,bool isShow) = 0;
        
        virtual void showUserOffLine(BYTE seatNo , bool offLine) = 0;
