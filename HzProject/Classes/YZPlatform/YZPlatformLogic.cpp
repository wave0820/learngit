...
namespace YZ
{
	...
	void YZPlatformLogic::platformLogin(const std::string& name, const std::string& password, UINT uNameID)
	{
		...
		logonByName.uRoomVer = PLATFORM_CLINT_VERSION;
        memset(logonByName.szUID, 0, sizeof(logonByName.szUID));
        memset(logonByName.szHeadUrl, 0, sizeof(logonByName.szHeadUrl));
		...
		logonByName.gsqPs = PLATFORM_CLINT_GSQPS;
	...
	void YZPlatformLogic::onLoginMessage(NetMessageHead* messageHead, void* object, INT objectSize)
	{
		...
		case ASS_GP_LOGON_ERROR:
			{
                MSG_GP_R_UPDATE_URL* urlInfor = (MSG_GP_R_UPDATE_URL*)object;
                ...
                dispatchMessage([&](IPlatformNotifyDelegate* delegate) -> bool
                {
                    delegate->onHandleLoginMessage(_platformLogin, urlInfor,messageHead->bHandleCode);
                    return false;
                });
			}
			break;
