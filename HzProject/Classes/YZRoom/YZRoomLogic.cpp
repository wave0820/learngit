...
namespace YZ
{
        ...
        }else if (ASS_GR_CREATE_BACKROOM == messageHead->bAssistantID)	 // 创建房间
        {
            ...
            if (count > 0 && code == 0)
            {
                ...
                dispatchFrameMessage([data,code](IRoomMessageDelegate* delegate) -> bool
                                     {
                                         delegate->onCreateBackRoomMessage(data , code);
                                         return false;
                                     });
            }else{
                CHAR buf[512];
    ...
    void YZRoomLogic::setDeskOwnerInfo(int userId){
        JzDebug("添加桌主信息 ---- 所有者 : %d "  , userId);
        _roomDeskOwner = userId;
    }
    
    int YZRoomLogic::getDeskOwner(){
        return _roomDeskOwner;
    }
    
    void YZRoomLogic::clearDeskOwnerInfo(){
        _roomDeskOwner = -1;
    }
}
