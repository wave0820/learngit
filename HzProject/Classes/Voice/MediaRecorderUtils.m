...
+ (void)init:(NSString *)filePath AndFileName:(NSString *)fileName
{
    ...
    _amrPath =  [[filePath stringByAppendingString:fileName ]stringByAppendingString:@".amr"];
...
+ (void)playRecord:(NSString *)filePath AndFileName:(NSString *)fileName{
    NSString * amrPath =  [[filePath stringByAppendingString:fileName ]stringByAppendingString:@".amr"];
    //NSURL *url = [NSURL fileURLWithPath:amrPath];
    //NSLog(@"播放录音:%@",url);
    NSString * wavPath =  [[filePath stringByAppendingString:fileName ]stringByAppendingString:@".wav"];
    [amrPath retain];
    [wavPath retain];
    [VoiceConverter amrToWav:amrPath wavSavePath:wavPath];
    NSLog(@"playRecord Wav Path:%@",wavPath);
    [[MediaRecorderUtils alloc] _startPlayRecord:wavPath];
    [amrPath release];
    [wavPath release];
}


- (void)_startPlayRecord:(NSString *)filePath
{
